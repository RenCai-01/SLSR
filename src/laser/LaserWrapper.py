...
# from laser.llama_laser import LLAMALaser
...
from laser.mistral_laser import MistralLaser  #2025年1月19日00:02:19增加
...
class LaserWrapper:
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        #                                         logger=logger,
        #                                         in_place=in_place)
        if type(model) == MistralForCausalLM:
            logger.log("Editing a MistralForCausalLM Model")

            return MistralLaser.get_edited_model(model=model,
                                                lname=lname,
                                                lnum=lnum,
                                                rate=rate,
                                                intervention=intervention,

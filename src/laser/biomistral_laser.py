...
class BioMistralLaser(AbstractLaser):
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        converted_names = BioMistralLaser.convert_name(lname)
        num_update = 0

        for name, param in model.named_parameters():

            modify_flag = BioMistralLaser._modify_layer(name=name,
                                                    lnum_to_modify=lnum,
            ...
            BioMistralLaser.update_model(model_edit, name, mat_analysis)
            num_update += 1

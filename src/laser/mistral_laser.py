...
from laser.matrix_utils import do_low_rank, sorted_mat, prune, add_noise
...
class MistralLaser(AbstractLaser):
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        converted_names = MistralLaser.convert_name(lname)
        num_update = 0

        for name, param in model.named_parameters():

            modify_flag = MistralLaser._modify_layer(name=name,
                                                    lnum_to_modify=lnum,
            ...
            elif intervention == 'rank-reduction':  #注意这里，这里把param引进来，就是要改了
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                # mat_analysis = do_low_rank(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
                # mat_analysis, model_edit_new= do_low_rank(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1, model_edit)  # 改 2024年10月25日16:45:53
                # mat_analysis, model_edit_new, U,S,V= do_low_rank(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1, model_edit)
                mat_analysis,U,S,V = do_low_rank(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)  # 2024年10月30日17:39:33
            elif intervention == 'add-noise':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = add_noise(mat_analysis_tensor.type(torch.float32), 0)
            ...
            # MistralLaser.update_model(model_edit, name, mat_analysis)
            MistralLaser.update_model(model_edit, name, mat_analysis)
            num_update += 1

        assert num_update > 0, f"Must update some parameters Mistral: {lnum}, {lname}"
        ...
        # 这里有个疑问，上述MistralLaser.update_model之后好像没有直接返回model_edit，不知道下面一句return model_edit的是不是做了有效修改
        #通过copymodifyceshi.py这个文件可知，实际上经过update_model函数之后，model_edit已经被修改，并且下面再次提到的model_edit的也是修改过的
        # return model_edit
        # return model_edit,model_edit_new,U,S,V  # 任改 2024年10月25日17:05:49

        if intervention == 'rank-reduction':
            return model_edit,U,S,V  # 任改 2024年10月30日17:38:16
        elif intervention == 'add-noise':
            return model_edit  # 任改 2025年5月31日04:23:52
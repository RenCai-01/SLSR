...
if __name__ == '__main__':
    ...
    cat_cors = {cat: [] for cat in categories}  # 从categories中可以看出，因为college_medicne属于subcategories而不是categories，所以不需要这块了
    ...
    for subject in subjects:
        ...
        if 1:
            dev_df = pd.read_csv(
                os.path.join(args.csv_data_dir, "dev", subject + "_dev.csv"), header=None
            )[: args.ntrain]
            test_df = pd.read_csv(
                os.path.join(args.csv_data_dir, "val", subject + "_val.csv"), header=None
            )
